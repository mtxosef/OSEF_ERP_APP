...
var gpExploradorConceptos_ItemClick = function (gridview, registro, gvhtml, index) {
    ...
    window.parent.App.wEmergente.setHeight(490);
    ...
    //Id mov
    Ext.util.Cookies.set('cookieID', registro.get('RMovimiento').Id);
...
//Evento que hace el filtro al seleccionar algun elemento
var txtMov_Change = function (textfield, newValue, oldValue, e) {
    App.sPreciarioConcepto.clearFilter();
    App.sPreciarioConcepto.filter([{ filterFn: function (item) {

        if (item.get('RMovimiento').MovID.toUpperCase().indexOf(newValue.toUpperCase()) > -1) {
            return true;
        }
        else {
            return false;
